...
router.post('/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  // Set a timeout for webhook processing to prevent Stripe timeouts
  const webhookTimeout = setTimeout(() => {
    if (!res.headersSent) {
      console.log('>>> Webhook timeout - responding with 200 to prevent retries');
      res.status(200).json({ received: true, timeout: true });
    }
  }, 8000); // 8 second timeout (Stripe timeout is 10 seconds)
  ...
  try {
    ...
    try {
          ...
          try {
            ...
            if (dbOrderId) {
              ...
              if (updatedOrder) {
                ...
                // Send restaurant notification email (ASYNC - don't wait)
                setImmediate(async () => {
                  try {
                ...
                // Send customer order confirmation email (ASYNC - don't wait)
                if (updatedOrder.customer_email) {
                  setImmediate(async () => {
                    try {
                      console.log('>>> Sending customer order confirmation email to:', updatedOrder.customer_email);
                      ...
                      for (const item of orderItems) {
                        ...
                        if (menuItem) {
                          // Get restaurant name from the first menu item
                          if (restaurantName === 'Restaurant') {
                            const restaurant = await restaurantRepository.findOne({
                              where: { id: menuItem.restaurant_id }
                            });
                            if (restaurant) {
                      ...
                      const emailResult = await sendOrderConfirmationEmail(
                        {
                          ...
                          subtotal: (updatedOrder.total || 0) - (updatedOrder.tax || 0)
                        },
                      ...
                      if (emailResult.success) {
                        ...
                        logger.info('Customer order confirmation email sent successfully', {
                          correlationId,
                          context: 'payments.webhook',
                          data: {
                            orderId: updatedOrder.id,
                      ...
                      console.log('>>> Exception sending customer order confirmation email:', emailError);
                      logger.error('Exception sending customer order confirmation email', {
                        correlationId,
                        context: 'payments.webhook',
                  ...
                  console.log('>>> No customer email found for order:', updatedOrder.order_number);
                  logger.warn('No customer email found for order confirmation', {
                    correlationId,
                    context: 'payments.webhook',
                    data: {
                      orderId: updatedOrder.id,
      ...
      // Clear timeout and respond quickly
