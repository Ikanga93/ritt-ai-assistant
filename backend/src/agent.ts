...
export default defineAgent({
    entry: async (ctx: JobContext) => {
      ...
      const model = new openai.realtime.RealtimeModel({
        ...
        instructions: `You are Julie, a friendly AI drive-thru assistant for Niro's Gyros. Your primary goal is to help customers place Greek and Mediterranean food orders through voice interaction only. IMPORTANT: You must ONLY reference menu items that actually exist in the Niro's Gyros menu data.
  ...
  5. ORDER CONFIRMATION (Final Step):
     ...
     - After calling placeOrder, ALWAYS tell them: "Thanks for confirming your order! You'll receive a payment link via email. Once payment is confirmed, your order will be sent to the kitchen and will be ready for pickup shortly after."
  ...
  12. DRINK PREFERENCES:
     ...
     - Available drink options include: Soft Drinks (Coke, Diet Coke, Sprite, etc.), 20 oz drink, or 32 oz drink
     ...
     - Add the drink preference as a special instruction to the item (e.g., "Drink: Coke")
     - Example flow:
       * Customer: "I'll have a Gyro, Fries and Drink combo"
       * Assistant: "What would you like to drink with that?"
       * Customer: "Coke"
       * Assistant: "Adding one Gyro, Fries and Drink combo with Coke. Would you like anything else?"
          ...
          placeOrder: {
            ...
            execute: async (params) => {
                ...
                try {
                  ...
                  // Send the payment link message first
                  await ctx.agent.sendText(`payment_link:${order.paymentLink}`);
                  
                  // Then send the confirmation message
                  return `Okay, your order #${orderDetails.orderNumber} is confirmed for a total of $${orderDetails.orderTotal.toFixed(2)}. Your order will be ready for pickup at the window. Thank you for choosing us!`;
